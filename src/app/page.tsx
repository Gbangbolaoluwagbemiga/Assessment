...
export default function AssessmentApp() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const isDone =
        localStorage.getItem("web3bridge_assessment_done") === "true";
  ...
  const fetchRepos = async (username: string): Promise<boolean> => {
    ...
    try {
      ...
      const filtered = data.filter(
        (repo) => new Date(repo.created_at) < ninetyDaysAgo,
      );
      ...
      toast.error(
        isTimeout ? "GitHub Verification Timeout" : "GitHub Network Error",
        {
  ...
  const submitIntake = async (e: React.FormEvent) => {
    ...
    if (
      ...
      return toast.error(
        "Please fill all required fields including GitHub Username.",
      );
    }
    ...
    const unrated = coreConcepts.filter(
      (c) => !formData[c.key as keyof typeof formData],
    );
    if (unrated.length > 0) {
      return toast.error(
        `Please rate your familiarity with ${unrated[0].label}`,
      );
    }
    ...
    try {
      ...
      if (qList.length > 0) {
        ...
        throw new Error(
          "Evaluation engine returned no questions. Contact support.",
        );
      }
    } catch (err: any) {
      toast.error("Assessment Engine Failed", {
        description:
          err.message || "Failed to load questions. Check your connection.",
        duration: Infinity,
        action: { label: "Dismiss", onClick: () => {} },
      });
      setStep("intake");
  ...
  const nextQuestion = () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      toast.error("Answer required", {
        description:
          "Please provide an answer to proceed to the next challenge.",
        duration: 3000,
      });
    }
  };
        ...
        setTimeRemaining((prev) => {
          if (prev === 181 && !hasWarned3Min) {
            toast.warning("Critial Update: 3 minutes remaining!", {
              description:
                "The evaluation engine will auto-submit when the timer hits zero.",
              duration: Infinity,
              action: { label: "Dismiss", onClick: () => {} },
            });
    ...
    return parts.map((part, i) => {
      ...
      if (blockMatch) {
        ...
        return (
          <pre
            key={i}
            className="block w-full bg-[#0d0d0d] border border-white/5 rounded-2xl p-8 my-8 font-mono text-xl text-indigo-400 overflow-x-auto shadow-2xl leading-relaxed custom-scrollbar"
          >
            {block.split("\n").map((line, j) => (
              <div key={j} className="flex gap-4">
                <span className="text-gray-700 select-none w-6 text-right text-sm mt-1">
                  {j + 1}
                </span>
                <span>{line}</span>
              </div>
      ...
      <AnimatePresence mode="wait">
        {isAlreadyCompleted ? (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="w-full max-w-2xl bg-[#0A0A0F]/60 backdrop-blur-2xl border border-white/5 shadow-2xl rounded-[2.5rem] p-12 text-center"
          >
            ...
            <p className="text-gray-500 text-lg font-medium leading-relaxed">
              Our records show that you have already completed the Web3bridge
              technical intake. Multiple attempts are strictly prohibited to
              ensure candidate integrity.
            </p>
          </motion.div>
        ) : (
            ...
            <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar space-y-12">
              {step === "intake" && (
                ...
                >
                  <div className="text-center mb-12 py-6">
                    <h1 className="text-3xl font-black mb-3 tracking-tight text-white uppercase italic">
                      Developer Assessment Form
                    </h1>
                    <p className="text-gray-400 text-lg font-medium tracking-tight">
                      Initial profile setup for the Web3bridge evaluation
                      engine.
                    </p>
                  </div>
                  ...
                  <form onSubmit={submitIntake} className="space-y-12">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <label className="block text-[10px] text-gray-500 mb-2 font-black uppercase tracking-widest">
                            Full name *
                          </label>
                          <input
                            required
                            ...
                            className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 focus:border-indigo-500 transition-all text-base font-bold text-white placeholder-gray-800"
                        ...
                        <div className="md:col-span-2">
                          ...
                          <div className="relative group">
                            <input
                              ...
                              className="w-full bg-black/40 border border-white/5 rounded-xl pl-10 pr-4 py-3 focus:border-indigo-500 transition-all text-lg font-black text-white placeholder-gray-800"
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <select
                            required
                            className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 focus:border-indigo-500 transition-colors text-base font-bold text-white appearance-none"
                        ...
                        <div>
                          ...
                          <input
                            ...
                            min="0"
                            placeholder="03"
                            className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 focus:border-indigo-500 transition-colors text-base font-bold text-white placeholder-gray-800"
                      ...
                      <div>
                        <label className="block text-[10px] mb-3 text-gray-500 font-black uppercase tracking-widest">
                          Self-assess skill *
                        </label>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                          {[
                            ...
                            <label
                              key={l}
                              className={`flex items-center gap-3 cursor-pointer p-4 rounded-xl border transition-all ${formData.proficiency === l.toLowerCase() ? "border-indigo-500 bg-indigo-500/10 text-white" : "border-white/5 bg-white/5 hover:bg-white/10 text-gray-600"}`}
                            >
                              <div
                                className={`w-4 h-4 rounded-full border flex items-center justify-center transition-all ${formData.proficiency === l.toLowerCase() ? "border-indigo-400" : "border-white/20"}`}
                              >
                                {formData.proficiency === l.toLowerCase() && (
                        ...
                        <table className="w-full text-left border-collapse min-w-[600px]">
                          <thead>
                            <tr className="bg-white/5">
                              <th className="py-4 px-6 font-black text-white uppercase text-[10px] tracking-widest w-1/3">
                                Technical Domain
                              </th>
                              {conceptOptions.map((opt) => (
                                <th
                                  key={opt}
                                  className="py-4 px-2 text-center font-black text-white uppercase text-[10px] tracking-widest"
                                >
                                  {opt}
                                </th>
                              ))}
                          ...
                          <tbody className="divide-y divide-white/5">
                            {coreConcepts.map((concept, idx) => (
                              ...
                              >
                                <td className="py-5 px-6 font-bold text-sm text-white italic">
                                  {concept.label}
                                </td>
                                {conceptOptions.map((opt) => (
                                  <td
                                  ...
                                  >
                                    <label className="inline-flex cursor-pointer items-center justify-center p-2 group w-full h-full">
                                      <div
                                        className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${formData[concept.key as keyof typeof formData] === opt.toLowerCase() ? "border-indigo-500 bg-indigo-500" : "border-white/10 group-hover:border-white/30"}`}
                                      >
                                        {formData[
                                          concept.key as keyof typeof formData
                                        ] === opt.toLowerCase() && (
                                      ...
                                      <input
                                        type="radio"
                                        name={`concept_${concept.key}`}
                                        className="hidden"
                                        value={opt.toLowerCase()}
                                        onChange={() =>
                                          setFormData({
                                            ...formData,
                                            [concept.key]: opt.toLowerCase(),
                                          })
                                        }
                                      />
                                    </label>
                    ...
                    <div className="pt-10 flex justify-center pb-12">
                      <button
                        ...
                        className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-black rounded-xl px-12 py-4 transition-all shadow-xl shadow-indigo-500/20 flex items-center gap-4 text-sm uppercase italic transform active:scale-95"
                      >
                        {isProcessing ? (
                          <Loader2 className="w-5 h-5 animate-spin" />
                        ) : (
                          <>
                            Load Questions <ChevronRight className="w-5 h-5" />
                          </>
                        )}
                      </button>
                    </div>
              ...
              {step === "assessment" && curQ && (
                ...
                >
                  ...
                  <div className="flex justify-between items-center mb-10 pb-6 border-b border-white/5">
                    ...
                    <div
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg font-mono font-black border transition-all text-sm tracking-tighter ${timeRemaining <= 180 ? "text-rose-400 bg-rose-400/5 border-rose-400/20 animate-pulse" : "text-emerald-400 bg-emerald-400/5 border-emerald-500/20"}`}
                    >
                    ...
                    <div className="w-full space-y-8">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                          {curQ.options?.map((opt: string, i: number) => {
                            ...
                            return (
                              ...
                              >
                                <div
                                  className={`w-7 h-7 rounded-lg border flex items-center justify-center font-black text-xs transition-all ${answers[curQ.id] === letter ? "bg-indigo-500 border-indigo-400 text-white" : "bg-black/20 border-white/10 text-gray-600"}`}
                                >
                                  {letter}
                                </div>
                                <span
                                  className={`${answers[curQ.id] === letter ? "font-bold" : "text-gray-300"}`}
                                >
                                  {opt}
                                </span>
                              </button>
                      ...
                      {curQ.type === "truefalse" && (
                        <div className="flex gap-4 max-w-sm mx-auto">
                          {(curQ.options && curQ.options.length === 2
                            ? curQ.options
                            : ["True", "False"]
                          ).map((val: any) => {
                      ...
                      {(curQ.type === "fillintheblank" ||
                        curQ.type === "fillinblank") && (
                  ...
                  <div className="mt-16 pt-8 border-t border-white/5 flex justify-between items-center pb-10">
                    <button
                      ...
                      className="px-6 py-3 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition disabled:opacity-10 text-white font-bold text-[10px] uppercase"
                    >
                      Back
                    </button>
                    <button
                      onClick={nextQuestion}
                      className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 rounded-xl font-black transition-all flex items-center gap-3 text-white text-sm uppercase italic active:scale-95"
                    >
              ...
              {step === "repo" && (
                ...
                >
                  <div className="text-center mb-10">
                    <h2 className="text-3xl font-black mb-4 text-white uppercase italic">
                      Verification Engine
                    </h2>
                    <p className="text-gray-500 text-base max-w-xl mx-auto">
                      Identify your benchmark project for architectural review.
                      Mandatory 3-month age requirement applies.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pb-12">
                    {repos.length > 0 ? (
                      repos.map((repo) => (
                        ...
                        >
                          ...
                          <p className="text-xs text-gray-600 mt-2 line-clamp-2">
                            {repo.description || "Verified public repository."}
                          </p>
                          <div className="flex items-center justify-between mt-auto">
                            ...
                            <div className="text-[10px] font-bold text-amber-500/50 flex items-center gap-1">
                      ...
                      <div className="col-span-full py-20 text-center border border-dashed border-white/5 rounded-2xl">
                        <p className="text-gray-700 text-sm font-bold uppercase tracking-widest">
                          No matching benchmark repositories found.
                        </p>
                      </div>
                    )}
                  </div>
                    ...
                    <button
                      onClick={async () => {
                        if (!selectedRepo && repos.length > 0)
                          return toast.error("Select a repository.");

                        setIsProcessing(true);
                        setStep("loading");
                        setLoadingText(
                          "> Finalizing architectural evaluation...",
                        );
                        ...
                        try {
                          const finalPayload = {
                            ...
                            results: Object.entries(answers).map(
                              ([id, ans]) => ({
                                questionId: id,
                                answer: ans,
                              }),
                          ...
                          await fetch(
                            "https://assessment-n8n.web3bridge.com/webhook/submit",
                            {
                              method: "POST",
                              headers: { "Content-Type": "application/json" },
                          ...
                          toast.error("Submission Failure", {
                            description:
                              "Failed to sync metrics with evaluation cloud. Check your network.",
                            duration: 5000,
                          });
                          setStep("repo");
                        } finally {
              ...
              {step === "success" && (
                ...
                >
                  ...
                  <h2 className="text-4xl font-black mb-4 text-white uppercase italic">
                    Assessment Logged
                  </h2>
                  <p className="text-gray-500 max-w-md mx-auto text-lg font-medium italic">
                    The evaluation engine has recorded your phase metrics. You
                    will be notified via the primary mailbox provided.
                  </p>
                </motion.div>
              )}
            </div>
